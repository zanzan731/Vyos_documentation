...
\begin{longtable}{|l|l|l|l|}
\hline
\textbf{Interface} & \textbf{Announced prefix} & \textbf{RA flags} & \textbf{SLAAC enabled?} \\
\hline
...
Notes:
- For IPv4 we use DHCP with static DHCPv4 maps for servers and important hosts (match by MAC address).
- For IPv6 SLAAC is enabled on INTERNAL and IPV6ONLY so clients auto-configure addresses from the announced prefix. Use DHCPv6 static maps only for hosts that require a stable, administratively assigned address (servers, controllers).
- If an interface advertises both \texttt{managed} and \texttt{no-autonomous}, SLAAC is effectively disabled and hosts must obtain an address via DHCPv6.
...
\begin{itemize}
  \item DMZ: DHCPv6 is used for servers (range \texttt{2001:1470:fffd:a9::100}--\texttt{::1ff}) and static DHCPv6 maps are configured for important hosts.
  \item INTERNAL: SLAAC is the default for clients on \texttt{2001:1470:fffd:aa::/64}; use DHCPv6 static maps only for servers that require a fixed IPv6 address.
  \item IPV6ONLY: SLAAC is the default on \texttt{fd11:11:11::/64}; these hosts autoconfigure addresses and do not normally require DHCPv6 static mappings.
\end{itemize}

The DHCPv6 server address for DMZ static allocations remains \texttt{2001:1470:fffd:a9::1}.
