...
\begin{itemize}
  ...
  \item Izpostaviti navzven le izbrane storitve: WireGuard in REST frontend (HTTPS).
...
\href{https://github.com/zanzan731/Vyos\_documentation/}{https://github.com/zanzan731/Vyos\_documentation/}
...
\begin{figure}[ht]
  ...
  \caption{Posnetek imen virtualk}
\end{figure}
...
\begin{itemize}
  ...
  \item Internal Linux virtualke uporabljajo \textbf{Ubuntu Desktop 26.04}.
  \item Internal Windows virtualke uporabljajo \textbf{Windows 10}.
  \item IPv6-only Linux virtualke uporabljajo \textbf{Ubuntu Desktop 26.04}.
...
Na \texttt{internal} in \texttt{ipv6only} odjemalcih je lokalni uporabnik vedno samo \texttt{zanzan}, drugi uporabniki so pridobljeni od AD.
...
\subsection{SSH}
SSH je konfiguriran za prijavo prek ključev (avtentikacija z geslom je onemogočena) na privzetem portu 22. Ključe dodajte v \texttt{~/./ssh/authorized\_keys}.
...
\hline
\textbf{Storitev} & \textbf{Gostitelj} & \textbf{Port (proto)} & \textbf{Javno?} & \textbf{Opombe} \\
\hline
...
\hline
\texttt{snmp\_exporter} & snmp & 9116 (tcp) & Ne & Končna točka exporterja za Prometheus \\
\hline
...
\subsection{Oglasi usmerjevalnika (RA)}
RA (oglasi usmerjevalnika) po vmesnikih in stanju SLAAC:
\begin{longtable}{|l|l|l|l|}
\hline
\textbf{Vmesnik} & \textbf{Oglaševana predpona} & \textbf{RA zastavice} & \textbf{SLAAC} \\
\hline
...
Opombe:
\begin{itemize}
  \item Za IPv4 uporabljamo DHCP s statičnimi DHCPv4 mape za strežnike in pomembne gostitelje (ujemanje po MAC naslovu).
  \item Za IPv6 je na INTERNAL omogočen SLAAC; IPV6ONLY in DMZ uporablja DHCPv6, odjemalci pa naslove pridobijo prek DHCPv6, razen če je oglašen avtonomen predpon.
  \item Če vmesnik oglašuje tako \texttt{managed} kot \texttt{no-autonomous}, je SLAAC dejansko onemogočen in gostitelji morajo naslov pridobiti prek DHCPv6.
\end{itemize}
...
\textbf{OPOMBA:} niso vsi ukazi idempotentni! Za nekatere ukaze set v resnici pomeni add...
npr če bi na ta način aplicirali celoten config, boste gotovo dobili podvojene omrežne vmesnike ali kakšne napake.
Zato je boljše to uporabljati kot način da aplicirate neki blok novih ukazov, ne spreminjati obstoječo konfiguracijo.
...
Ukazi za administracijo AD DNS strežnika so zbrani v repozitoriju v datoteki \texttt{ad-dns-upravljanje.md}.

\section{DHCP}

\subsection{DHCPv4}
DHCP server ima dve skupini: \textbf{SERVERS} (192.168.11.0/24) in \textbf{USERS} (10.11.0.0/24).
Obe skupini uporabljata statične DHCP mape, nedodeljeni uporabniki pa dobijo naslov avtomatsko.
...
\begin{itemize}
  \item DMZ: DHCPv6 uporabljamo za strežnike (razpon \texttt{2001:1470:fffd:a9::100}--\texttt{::1ff})
  \item INTERNAL: SLAAC je privzeta izbira za odjemalce na \texttt{2001:1470:fffd:aa::/64}
  \item IPV6ONLY: za \texttt{fd11:11:11::/64} uporabljamo DHCPv6 statične mape da bi uporabniki lahko dostopali do oddaljenega omizja
\end{itemize}
...
\begin{longtable}{|l|l|l|}
\hline
\textbf{Gostitelj} & \textbf{DHCPv6 naslov} & \textbf{DUID / identifikator} \\
\hline
...
\begin{longtable}{|l|l|l|l|p{2.4cm}|l|l|}
\hline
\textbf{Pravilo} & \textbf{Opis} & \textbf{In} & \textbf{Destination} & \textbf{Port} & \textbf{Prot} & \textbf{Prevod / cilj} \\
\hline
\endfirsthead
\hline
\textbf{Pravilo} & \textbf{Opis} & \textbf{In} & \textbf{Destination} & \textbf{Port} & \textbf{Proto} & \textbf{Prevod / cilj} \\
\hline
\endhead
...
\hline
\textbf{Pravilo} & \textbf{Opis} & \textbf{Out} & \textbf{Source} & \textbf{Destination} & \textbf{Proto/Notes} & \textbf{Prevod / opomba} \\
\hline
\endfirsthead
\hline
\textbf{Pravilo} & \textbf{Opis} & \textbf{Out} & \textbf{Source} & \textbf{Destination} & \textbf{Proto/Notes} & \textbf{Prevod / opomba} \\
...
\begin{itemize}
  \item \textbf{Model:} privzeto-zavrni (default-deny) — vse povezave so privzeto prepovedane, razen če so izrecno dovoljene.
  \item \textbf{Kaj je dovoljeno:} vse naprave lahko dostopajo do interneta in vseh DMZ storitev (DNS, web, monitoring, AD). ICMP/ICMPv6 za diagnostične potrebe je povsod dovoljen.
  ...
  \item \textbf{Hairpin NAT:} notranji in DMZ odjemalci lahko dosežejo javne IPv4 storitve preko hairpin DNAT/SNAT.
\end{itemize}
...
\begin{longtable}{|l|l|l|l|l|p{2.2cm}|l|}
\hline
...
\begin{longtable}{|l|l|l|p{2.2cm}|l|}
\hline
...
\hline
\endhead
10 & accept & allow-router-icmpv6-output & eth0 &  &  &  &  & icmpv6 \\
...
\begin{itemize}
  ...
  \item \textbf{Javni port:} UDP 51820
  \item \textbf{Javni dostop:} 88.200.24.241 prek obstoječe DNAT nastavitve
\end{itemize}
...
Monitoring v tej zasnovi je namenjen predvsem nadzoru omrežnega stanja, ne operativnemu upravljanju storitev. Trenutno edini vir metrik je VyOS usmerjevalnik prek SNMP,
iz katerega se berejo osnovni podatki o vmesnikih, dosegljivosti, prometa in sistemskem stanju. Te metrike se nato pretvorijo v Prometheus format, zberejo v Prometheusu in vizualizirajo v Grafani. 
Monitoring stack je nameščen prek Dockerja (Docker Compose) na gostitelju za nadzor.

\subsection{Arhitektura spremljanja}
Monitoring gostitelj je 
\texttt{192.168.11.107}, kjer tečejo tri komponente:
\begin{itemize}
  \item \textbf{VyOS SNMP}: \texttt{192.168.11.1:161/udp} kot vir podatkov;
  \item \textbf{snmp\_exporter}: \texttt{192.168.11.107:9116/tcp} kot pretvornik SNMP podatkov v Prometheus metrike;
  \item \textbf{Prometheus}: \texttt{192.168.11.107:9090/tcp} kot zbiralnik in časovna baza metrik;
  \item \textbf{Grafana}: \texttt{192.168.11.107:3000/tcp} kot vizualizacijski sloj za nadzorne plošče.
\end{itemize}

\subsection{Kaj spremljamo (metrike in uporabljeni MIB-i)}
Monitoring zajema metrike, ki jih bere \texttt{snmp\_exporter} iz VyOS preko SNMP in jih Prometheus zbira ter agregira za vizualizacijo in alarmiranje. Spodaj je skrbno izbran nabor pomembnih metrik z označenimi MIB-i in osnovnimi OID-i (izvleček iz \texttt{snmp\_stack/snmp.yml}):
\begin{itemize}
  \item \textbf{Sistemske metrike (SNMPv2-MIB)}: \texttt{sysUpTime} — \texttt{1.3.6.1.2.1.1.3} (dosegljivost, reboot detection).
  \item \textbf{Vmesniki in identifikacija (IF-MIB)}: \texttt{ifNumber} (\texttt{1.3.6.1.2.1.2.1}), \texttt{ifIndex}, \texttt{ifDescr} (\texttt{1.3.6.1.2.1.2.2.1.2}) — seznam in poimenovanje vmesnikov.
  \item \textbf{Konfiguracijski atributi (IF-MIB)}: \texttt{ifMtu} (\texttt{1.3.6.1.2.1.2.2.1.4}), \\ \texttt{ifSpeed} (\texttt{1.3.6.1.2.1.2.2.1.5}), \texttt{ifPhysAddress}.
  \item \textbf{Stanje vmesnikov}: \texttt{ifAdminStatus} / \texttt{ifOperStatus} (operativna/administrativna stanja).
  \item \textbf{Prometni števci (IF-MIB / IFX)}: bajti in paketi — \texttt{ifInOctets} (\texttt{1.3.6.1.2.1.2.2.1.10}), \texttt{ifOutOctets} (\texttt{1.3.6.1.2.1.2.2.1.16}), ter 64-bit HC števcem \texttt{ifHCInOctets} / \texttt{ifHCOutOctets} (\texttt{1.3.6.1.2.1.31.1.1.1.6} / \texttt{1.3.6.1.2.1.31.1.1.1.10}) za visoke hitrosti.
  \item \textbf{Napake in odmetki (IF-MIB)}: \texttt{ifInErrors}, \texttt{ifInDiscards}, \texttt{ifOutErrors} — signalizacija degradacij linka.
  \item \textbf{Dodatni prometni števci (IFX)}: multicast/broadcast števci in \\ \texttt{ifHighSpeed} (\texttt{1.3.6.1.2.1.31.1.1.1.15}).
  \item \textbf{IP/TCP/UDP statistike (po potrebi)}: zbirne statistike iz IP/TCP/UDP-MIB-ov (retransmisije, portne agregacije ipd.) — vključimo le ob potrebi za specifične istrage.
  \item \textbf{Agregacije in alarmiranje}: v Prometheusu izpeljane metrike (\texttt{rate()}, \texttt{increase()}), percentili in pragovna opozorila za latence, packet-loss in throughput.
  \item \textbf{Zdravje zbiranja}: status \texttt{snmp\_exporter} ter Prometheus job (cilj UP/DOWN, scrape errors) in metrika za zadnji uspešen scrape.
\end{itemize}

Opomba: celoten, natančen seznam metrik in podrobne preslikave so v \texttt{snmp\_stack/snmp.yml}. Za spremembe urejajte \texttt{snmp\_stack/generator.yml} in ponovno zaženite generator (navodila v \texttt{snmp progress.md}).

\subsection{Docker Compose konfiguraicja}
\begin{lstlisting}
version: '3'
services:
  snmp\_exporter:
...
\subsection{Konfiguracija Prometheus-a}
\begin{lstlisting}
scrape_configs:
  - job_name: 'snmp'
    static_configs:
...
Če želite spremeniti snmp.yml konfiguracijo, uporabite \href{https://github.com/prometheus/snmp_exporter/tree/main/generator}{uradni generator za `snmp\_exporter`}, 
da ustvarite prilagojen `snmp.yml`.
...
Še bolj podrobna namestitev, konfiguracijski primeri in preizkusni koraki so opisani ločeno v datoteki \texttt{snmp progress.md}.


\section{Active Directory (Windows Server)}
\subsection{Osnovne informacije}
Active Directory domena je nastavljena na strežniku \textbf{sk11-dmz-windows-ad}. Strežnik deluje kot primarni Domain Controller (PDC) za domeno \texttt{startup11.local}.

\begin{itemize}
    \item \textbf{Ime strežnika:} sk11-dmz-windows-ad
    \item \textbf{Domena:} startup11.local
